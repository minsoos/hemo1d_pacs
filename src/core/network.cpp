...
void Network::buildAndValidate(){
    ...
    for (Index i=0; i < vessels_.size(); ++i) {
        ...
        if (!vesselIndexById_.emplace(id, i).second){
            throw std::runtime_error("Error vessel duplicated, id: " + std::to_string(id));
        }
    }
    for (Index i=0; i < nodes_.size(); ++i) {
        const Id id = nodes_[i].id();
        if (!nodeIndexById_.emplace(id, i).second){
            throw std::runtime_error("Error node duplicated, id: " + std::to_string(id));
        }
    }
    ...
    for (const Node& node : nodes_){
        if (node.connections().empty()){
            throw std::runtime_error("Node " + std::to_string(node.id()) + " has no vessel connections");
        }

        const bool isTerminal = node.kind() == NodeKind::Terminal;

        if (isTerminal && !(node.boundaryCondition().has_value())){
            throw std::runtime_error("Terminal node " + std::to_string(node.id()) + " has no boundary condition");
        }

        if (!isTerminal && node.boundaryCondition().has_value()){
            throw std::runtime_error("Junction node " + std::to_string(node.id()) + " shouldn't have a boundary condition");
        }

        if (isTerminal && node.boundaryCondition()->type == BoundaryConditionType::Prescribed &&
            node.boundaryCondition()->csvFile.empty()){
            throw std::runtime_error("Prescribed boundary condition at node " + std::to_string(node.id()) + " has no csv");
        }

        if (!node.bifurcationAnglesRad().empty() && node.bifurcationAnglesRad().size() != node.connections().size()-1){
            throw std::runtime_error("Bifurcation angles at node " + std::to_string(node.id()) + " don't match the node's number");
        }
        ...
        for (const VesselConnection& conn : node.connections()){
            ...
            if (it == vesselIndexById_.end()){
                throw std::runtime_error("Node " + std::to_string(node.id()) + " has as connection the node " + std::to_string(conn.vesselId) + " which doesn't exist");
            }
        ...
        for (const Vessel& vessel : vessels_){
            ...
            if (ends[0] == kInvalidId || ends[1] == kInvalidId){
                throw std::runtime_error("Vessel " + std::to_string(vessel.id()) + " has the node " + std::to_string(ends[0]) + " or " + 
...
const Vessel& Network::vessel(Id id) const {
    ...
    if (it == vesselIndexById_.end()){
        throw std::runtime_error("Vessel " + std::to_string(id) + " is unknown "); 
...
const Node& Network::node(Id id) const {
    ...
    if (it == nodeIndexById_.end()){
        throw std::runtime_error("Node " + std::to_string(id) + " is not listed"); 
...
Id Network::nodeIdAt(Id vesselId, VesselEnd end) const {
    ...
    if (it == vesselIndexById_.end()){
        throw std::runtime_error("Node " + std::to_string(vesselId) + " is not listed"); 
    }
